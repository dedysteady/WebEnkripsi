...
function enkripsi(){
    ...
    switch(tipe){
        case "ENKRIPSI DEKRIPSI" : enkripsiVigenere(); // memanggil nilai function enkripsiVigenere()
            break; 
    }
}
...
function enkripsiStringVigenere(x,key){
    ...
    for (i = 0; i < huruf.length; i++) {
        ...
        indexseterusnya = (parseInt(indexsekarang) + parseInt(key)) % 61; //untuk mengambil perhitungan algoritmanya, yaitu plaintext+key mod 36
...
var huruf = new Array("a", "b", "c", "d", "e", "f", "g", "h", "i", "j", "k", "l", "m", "n", "o", "p", "q", "r", "s", "t", "u", "v", "w", "x", "y", "z", "1", "2", "3", "4", "5", "6", "7", "8", "9", "0", "!", "@", "#", "$", "%", "^", "&", "(", ")", "-", "_", "+", "=", "[", "{", "}", "]", "|", "\", ";", ":", ",", "<", ".", ">");
...
function dekripsi(){
    ...
    switch(tipe){
        case "ENKRIPSI DEKRIPSI" : dekripsiVigenere(); // memanggil nilai function dekripsiVigenere()
            break;
    }
}
...
function dekripsiStringVigenere(x,key){
    ...
    for (i = 0; i < huruf.length; i++) {
        ...
        if(hitung<0){
            hitung += 61; // variabel hitung nilainya sama dengan 36
        }

        indexseterusnya = hitung % 61; // untuk var indexseterusnya atau index hasil = variabel hitung di modulus 36
